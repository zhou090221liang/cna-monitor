...
function help(args) {
    ...
    console.info(`version\t显示当前版本号`);
    console.info(`-v\t显示当前版本号`);
    console.info('');

    console.info(`backup\t备份当前配置，后面指定备份文件保存目录，如：cna-monitor backup /cna/backup/`);
    console.info('');

    console.info(`import\t导入配置，后面指定对应的备份文件地址，如：cna-monitor import /cna/backup/backup_20211025.cna`);
    console.info('');

    console.info(`config\t设置或更改全局配置信息`);
    console.info(`\t支持的参数：`);
    ...
    console.info(`email\t设置或更改邮件服务器配置`);
    console.info(`\t支持的参数：`);
...
function status() {
    try {
        ...
        if (pid) {
            ...
            if (fs.existsSync(timeFile)) {
                try {
                    const time = new Date(parseInt(fs.readFileSync(timeFile).toString().decrypt())).format();
                    msg += `(上次检查时间:${time})`;
                } catch (e) { }
            }
            console.warn(msg);
...
function keys() {
    try {
        let keys = ["目标\t别名\t状态\t时间"];
        if (fs.existsSync(keysFile)) {
            const _keys = JSON.parse(fs.readFileSync(keysFile).toString().decrypt());
            _keys.forEach(item => keys.push(`${item.target}\t${item.alias}\t${item.status || "未知"}\t${item.time || ""}`));
        }
        return keys;
    } catch (e) {
...
function backup(args) {
    try {
        ...
        const _path = path.join(args.path, `./backup_${packageJson.version}_${new Date().format('yyyyMMddhhmmssS')}.cna`);
        args.debug && console.info('备份文件保存位置：', _path);
        const bak = {
            backupid: 'cna-monitor',
            date: new Date().format('yyyy/MM/dd hh:mm:ss.S'),
            version: packageJson.version,
            random: random.getRandomStr(32)
        };
        args.debug && console.info('组装备份信息');
        if (fs.existsSync(keysFile)) {
            const keys = JSON.parse(fs.readFileSync(keysFile).toString().decrypt());
            bak.keys = keys.map(item => { return { target: item.target, alias: item.alias } });
        }
    ...
    try {
        try {
            const html = await axios({
                method: 'get',
                url: 'https://www.npmjs.com/package/cna-monitor'
            });
            const $ = toJquery(html.data);
            const newVersion = $($($('time').parent().parent()).find('span')[1]).html().split('<')[0];
            // console.info("最新版本：V" + newVersion, "当前版本：V" + packageJson.version);
            if (newVersion == packageJson.version) {
                console.info("当前已是最新版本！");
                process.exit();
            }
            let answer = readlineSync.question(`发现新版本V${newVersion}(当前版本V${packageJson.version})，请确认是否继续(y/n)？`);
            if (answer.toLowerCase() != 'y') {
                process.exit();
            }
        } catch (e) {
            console.warn("警告：检查最新版本失败，直接尝试强制升级")
        }
        let msg = "正在升级中";
        const { spawn } = require('child_process');
        slog(msg);
        const npm = spawn('npm', ["install", "--unsafe-perm=true", "--allow-rootcd", "cna-monitor", "-g"]);
