...
function init(args) {
    fs.writeFileSync(configFile, JSON.stringify({ t: args.t > 0 ? args.t : 1, p: args.p || 10, m: args.m || 1 }).encrypt());
...
function _import(args) {
    try {
        ...
        const answer = readlineSync.question(`该备份文件备份于：${text.date}，继续将覆盖现有配置及监控目标等信息，请确认是否继续(y/n)？`);
